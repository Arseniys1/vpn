...
const StatsTab: React.FC = () => {
  const stats = [
    { label: 'Всего пользователей', value: '1,245', icon: 'fa-users', color: 'blue' },
    { label: 'Активных подписок', value: '892', icon: 'fa-check-circle', color: 'green' },
    { label: 'Доход (месяц)', value: '125,400 ★', icon: 'fa-coins', color: 'yellow' },
    { label: 'Открытых тикетов', value: '23', icon: 'fa-ticket', color: 'red' }
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-tg-secondary rounded-xl p-4">
        <h3 className="font-semibold text-tg-text mb-3">Активность серверов</h3>
        <div className="space-y-2">
          {['Германия', 'США', 'Нидерланды', 'Сингапур', 'Финляндия'].map((country, idx) => (
            <div key={idx} className="flex items-center justify-between">
              <span className="text-sm text-tg-text">{country}</span>
              <div className="flex items-center gap-2">
                <div className="w-32 h-2 bg-tg-bg rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-tg-blue rounded-full"
                    style={{ width: `${Math.random() * 100}%` }}
                  ></div>
                </div>
                <span className="text-xs text-tg-hint w-12 text-right">{Math.floor(Math.random() * 100)}%</span>
              </div>
            </div>
          ))}
        </div>
...
const ServersTab: React.FC = () => {
  const [servers, setServers] = useState([
    { id: '1', name: 'DE-1', country: 'Германия', flag: '🇩🇪', status: 'online', ping: 45, protocol: 'vless', adminMessage: 'Тестовое сообщение' },
    { id: '2', name: 'US-EAST', country: 'США', flag: '🇺🇸', status: 'online', ping: 120, protocol: 'vmess', adminMessage: '' },
    { id: '3', name: 'NL-VIP', country: 'Нидерланды', flag: '🇳🇱', status: 'crowded', ping: 38, protocol: 'vless', adminMessage: 'VIP сервер' }
  ]);
  const [editingServer, setEditingServer] = useState<string | null>(null);
  const [isAddingServer, setIsAddingServer] = useState(false);
  const [newServer, setNewServer] = useState({ name: '', country: '', flag: '', status: 'online', ping: 0, protocol: 'vless', adminMessage: '' });

  const handleSaveServer = (id: string, updatedData: any) => {
    setServers(servers.map(s => s.id === id ? { ...s, ...updatedData } : s));
  ...
  const handleAddServer = () => {
    ...
    setNewServer({ name: '', country: '', flag: '', status: 'online', ping: 0, protocol: 'vless', adminMessage: '' });
      ...
      {isAddingServer && (
        <div className="bg-tg-secondary rounded-xl p-4 space-y-3">
          ...
          <input
            ...
            className="w-full bg-tg-bg border border-tg-separator rounded-lg p-2 text-sm text-tg-text"
            placeholder="Ping (ms)"
            value={newServer.ping}
            onChange={(e) => setNewServer({ ...newServer, ping: parseInt(e.target.value) })}
          />
          <textarea
            ...
            rows={2}
            value={newServer.adminMessage}
            onChange={(e) => setNewServer({ ...newServer, adminMessage: e.target.value })}
          />
            ...
            <div className="p-4">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-tg-hint">{server.name} • {server.protocol.toUpperCase()} • {server.ping}ms</div>
                </div>
